...
private const val IMPORT_TAG = """import\s+\{?\s*(\w+)[\w\s,]*\}?\s+from\s+['"][^'"]+['"];?"""
...
class ExtractSvelteComponent(private val parser: SvelteParser? = null) {
    fun execute(request: ExtractRequest): ExtractResult {
        ...
        var modifiedSourceCode = callNewComponent()

        fun importStatements() {
        ...
        fun generateNewComponent(): String {
            val scriptTag = Regex(SCRIPT_TAG).find(request.sourceCode)
            val scriptAttributes = scriptTag?.groupValues?.get(1) ?: ""

            val importRegex = Regex(IMPORT_TAG)
        ...
        val newComponentContent = generateNewComponent()
